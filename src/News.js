...
export default function News() {
    ...
    const [mention, setMention] = useState(editingArticle ? editingArticle.mention : '');
    ...
    const openEditModal = (id) => {
        const articleToEdit = allArticles.find((item) => item._id === id) || pinnedArticles.find((item) => item._id === id);
        if (articleToEdit) {
            ...
            let fullFeaturedPlayers = articleToEdit?.featuredPlayers.map(featuredPlayerId => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const allArticlesIndex = allArticles.findIndex((item) => item._id === editingArticle._id);
            const pinnedArticlesIndex = pinnedArticles.findIndex((item) => item._id === editingArticle._id);
...
window.location.reload(true)
    ...
    const deleteArticle = async (id) => {
        try {
            ...
            const updatedPinnedArticles = pinnedArticles.filter((item) => item?._id !== id);
            setAllArticles(updatedAllArticles);
            setPinnedArticles(updatedPinnedArticles);
    ...
    return (
        <div className="flex flex-col gap-[20px] px-[20px]">
                ...
                {pinnedArticles?.map((articleItem) => (
                    <div key={articleItem?._id} className="video-card relative w-full  overflow-hidden">
                        ...
                        <div className="p-[10px]">
                            ...
                            <p className="text-[16px] text-[#818181]">{articleItem?.description}</p>
                            <p className='text-[16px] text-[#818181]'>@ {articleItem?.description}</p>
                        </div>
                        <div onClick={() => toggleMenu(articleItem.id)} className="absolute top-[10px] right-[10px] hover:cursor-pointer bg-white rounded-[100%] w-[30px] h-[30px] flex items-center justify-center">
                            <FaEllipsisV />
                            {activeImageId === articleItem.id && (
                                <div className="video-menu">
                                    <p onClick={() => unpinArticle(articleItem?._id)} className="options">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M13 2L3 14H12L11 22L21 10H12L13 2Z" stroke="#0E0E0E" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        Unpin
                                    </p>
                                    <p className="options" onClick={() => openEditModal(articleItem?._id)}>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M13.748 20.4438H21.0006" stroke="#0E0E0E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            <path fillRule="evenodd" clipRule="evenodd" d="M12.78 3.79479C13.5557 2.86779 14.95 2.73186 15.8962 3.49173C15.9485 3.53296 17.6295 4.83879 17.6295 4.83879C18.669 5.46719 18.992 6.80311 18.3494 7.82259C18.3153 7.87718 8.81195 19.7645 8.81195 19.7645C8.49578 20.1589 8.01583 20.3918 7.50291 20.3973L3.86353 20.443L3.04353 16.9723C2.92866 16.4843 3.04353 15.9718 3.3597 15.5773L12.78 3.79479Z" stroke="#0E0E0E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            <path d="M11.0215 6L16.4737 10.1871" stroke="#0E0E0E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        Edit
                                    </p>
                                    <p className="options" onClick={() => deleteArticle(articleItem?._id)}>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M19.3238 9.4668C19.3238 9.4668 18.7808 16.2018 18.4658 19.0388C18.3158 20.3938 17.4788 21.1878 16.1078 21.2128C13.4988 21.2598 10.8868 21.2628 8.27881 21.2078C6.95981 21.1808 6.13681 20.3768 5.98981 19.0458C5.67281 16.1838 5.13281 9.4668 5.13281 9.4668" stroke="#0E0E0E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            <path d="M20.708 6.23828H3.75" stroke="#0E0E0E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            <path d="M17.4406 6.239C16.6556 6.239 15.9796 5.684 15.8256 4.915L15.5826 3.699C15.4326 3.138 14.9246 2.75 14.3456 2.75H10.1126C9.53358 2.75 9.02558 3.138 8.87558 3.699L8.63258 4.915C8.47858 5.684 7.80258 6.239 7.01758 6.239" stroke="#0E0E0E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        Delete
                                    </p>
                                </div>
                    ...
                    <div key={articleItem?._id} className="video-card relative w-full  overflow-hidden">
                        ...
                        <div className="p-[10px]">
                            ...
                            <p className='text-[16px] text-[#818181]'>
                                @ {articleItem?.featuredPlayers.map(playerId => {
                                    const player = players.find(u => u?._id === playerId);
                                    return player ? player.name : '';
                                }).filter(name => name).join(', ')}
                            </p>
                        </div>
                        <div onClick={() => toggleMenu(articleItem?._id)} className="absolute top-[10px] right-[10px] hover:cursor-pointer bg-white rounded-[100%] w-[30px] h-[30px] flex items-center justify-center">
                            <FaEllipsisV />
                            {activeImageId === articleItem?._id && (
                                <div className="video-menu">
                                    <p onClick={() => pinArticle(articleItem?._id)} className="options">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M13 2L3 14H12L11 22L21 10H12L13 2Z" stroke="#0E0E0E" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        Pin
            ...
            <Modal show={showModal} onHide={handleClose}>
                <p className="justify-center text-[#f33] text-center py-[20px] m-0 text-[26px] font-bold font-sfPro">{editingArticle ? 'Edit Article' : 'New Article'}</p>
                <p className="justify-center text-center text-[16px] font-sfPro">{editingArticle ? 'Edit the article details' : 'Add new article to it.'}</p>
                <Modal.Body>
                    ...
                    <Form>
                        ...
                        <Form.Group className="mb-3" controlId="formParagraph">
                            <Form.Label className="text-[16px] font-sfPro">Paragraph</Form.Label>
                            <Form.Control
                                as="textarea"
                                rows={3}
                        ...
                        <Form.Group className="mb-3" controlId="formMention">
                            <Form.Label className="text-[16px] font-sfPro">Mention Player</Form.Label>
                            <Select
                                isMulti
                ...
                <Modal.Footer>
                    ...
                    >
                        {editingArticle ? 'Update' : 'Upload'} {/* Change button text based on editing state */}
                    </span>
                </Modal.Footer>
            </Modal>
            {/* edit modal */}
