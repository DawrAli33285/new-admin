...
export default function UserManagement() {
    ...
    const handleAddNewPlayer = async () => {
        const newPlayer = {
            ...
            image: files.length > 0 ? URL.createObjectURL(files[0]) : 'https://static.vecteezy.com/system/resources/thumbnails/008/846/297/small_2x/cute-boy-avatar-png.png', // Use uploaded image or default avatar
    ...
    const filteredPlayers = players.filter(row => {
        ...
        const nameMatch = row?.auth?.name?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
        const emailMatch = row?.auth?.email?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
        const phoneMatch = row?.auth?.phoneNumber?.includes(searchQuery) || false;
        const positionMatch = row?.position?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
        const locationMatch = row?.location?.toLowerCase().includes(searchQuery.toLowerCase()) || false;

        return statusMatch && (nameMatch || emailMatch || phoneMatch || positionMatch || locationMatch);
    });
    ...
    const handleStatusChange = async (id, newStatus) => {
        ...
        try {
            let response = await axios.post(`${BASE_URL}/updateStatus`, { id, status: newStatus })
    ...
    const handleEditPlayer = async() => {
        ...
        formdata.append('birthPlace', newPlayerData.location+' '+newPlayerData.birthplaceState)
        formdata.append('socialLinks', JSON.stringify(newPlayerData?.profile?.socialLinks))
        formdata.append('academics', JSON.stringify(newPlayerData?.profile.academics))
        formdata.append('athleticaccomplishments', JSON.stringify(newPlayerData?.profile?.athleticaccomplishments
            ))
        ...
        <div className="w-full  p-[20px]">
            ...
            <div className="overflow-x-auto w-f">
                <Table striped bordered hover>
                    <thead>
                        <tr>
                            ...
                            <th className="text-left text-[16px] font-normal font-sfPro">Player Name</th>
                    ...
                    <tbody>
                        {currentPlayers?.map((row, index) => (
                            <tr key={row._id}>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row._id}</td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">
                                    <div className="flex items-center">
                                        <img src={row?.picture} alt={row?._id} className="w-[30px] h-[30px] rounded-full mr-[10px]" />
                                        <span>{row?.auth?.name}</span>
                                    </div>
                                </td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row?.auth?.email}</td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row?.auth?.phoneNumber}</td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row?.position?.toUpperCase()}</td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row?.location}</td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row?.auth?.createdAt}</td>
                                <td className="py-[21px] px-[20px] border-[1px] border-[#E5E5E5]">{row?.status}</td>
                                <td className="text-[14px] font-sfPro text-center">
                                    {row?.status === 'pending' ? (
                                        <span className="flex items-center justify-between w-full">
                                            <span className="hover:cursor-pointer rounded-[100%] w-[30px] flex m-0 items-center bg-[#F33] h-[30px] justify-center" onClick={() => handleStatusChange(row._id, 'active')}>
                                            ...
                                            <span className="hover:cursor-pointer bg-white rounded-[100%] w-[30px] flex m-0 items-center h-[30px] justify-center" onClick={() => handleRemovePlayer(row._id)}>
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <span className="flex w-full justify-center items-center mt-[6px] hover:cursor-pointer relative" onClick={() => toggleMenu(row._id)}><FaEllipsisH />
                                            {activeVideoId === row?._id && (
                                                <div className="video-menu z-[9999]">

                                                    <p className="options" onClick={() => { openEditModal(row) }}>
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M13.748 20.4438H21.0006" stroke="#0E0E0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M12.78 3.79479C13.5557 2.86779 14.95 2.73186 15.8962 3.49173C15.9485 3.53296 17.6295 4.83879 17.6295 4.83879C18.669 5.46719 18.992 6.80311 18.3494 7.82259C18.3153 7.87718 8.81195 19.7645 8.81195 19.7645C8.49578 20.1589 8.01583 20.3918 7.50291 20.3973L3.86353 20.443L3.04353 16.9723C2.92866 16.4843 3.04353 15.9718 3.3597 15.5773L12.78 3.79479Z" stroke="#0E0E0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                            <path d="M11.0215 6L16.4737 10.1871" stroke="#0E0E0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                        </svg>
                                                        Edit
                                                    </p>
                                                    <p className="options" onClick={() => handleRemovePlayer(row._id)}>
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M19.3238 9.4668C19.3238 9.4668 18.7808 16.2018 18.4658 19.0388C18.3158 20.3938 17.4788 21.1878 16.1078 21.2128C13.4988 21.2598 10.8868 21.2628 8.27881 21.2078C6.95981 21.1808 6.13681 20.3768 5.98981 19.0458C5.67281 16.1838 5.13281 9.4668 5.13281 9.4668" stroke="#0E0E0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                            <path d="M20.708 6.23828H3.75" stroke="#0E0E0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                            <path d="M17.4406 6.239C16.6556 6.239 15.9796 5.684 15.8256 4.915L15.5826 3.699C15.4326 3.138 14.9246 2.75 14.3456 2.75H10.1126C9.53358 2.75 9.02558 3.138 8.87558 3.699L8.63258 4.915C8.47858 5.684 7.80258 6.239 7.01758 6.239" stroke="#0E0E0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                        </svg>
                ...
                <Pagination className="justify-content-end">
                    ...
                    <Pagination.Next onClick={handleNextPage} disabled={endIndex >= filteredPlayers.length} />
                </Pagination>
            </div>
            <Modal show={showModal} onHide={handleCloseModal}>
                ...
                <Modal.Body>
                    <Form>
                        ...
                        {/* Stats */}
                        <Form.Group controlId="formStats" className="flex flex-col gap-[20px]">
                            <Form.Label>Stats</Form.Label>
                            <Form.Control
                                type="number"
                                placeholder="GP"
                                value={newPlayerData.gp}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, gp: e.target.value })}
                            />
                            <Form.Control
                                type="number"
                                placeholder="FG"
                                value={newPlayerData.fg}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData,fg: e.target.value })}
                            />
                            <Form.Control
                                type="number"
                                placeholder="Threep"
                                value={newPlayerData.threep}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, threep: e.target.value })}
                            />
                            <Form.Control
                                type="number"
                                placeholder="Ft"
                                value={newPlayerData.ft}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, ft: e.target.value })}
                            />
                            <Form.Control
                                type="number"
                                placeholder="Reb"
                                value={newPlayerData.reb}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, reb: e.target.value })}
                            />
                           ...
                           <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, ast: e.target.value })}
                            />
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, blk: e.target.value })}
                            />
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, stl: e.target.value })}
                            />
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, pf: e.target.value })}
                            />
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, to: e.target.value })}
                            />
                           ...
                           <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, pts: e.target.value })}
                            />
                        ...
                        {/* Contact Information */}
                        <Form.Group controlId="formContactInformation" className=" mt-[10px] flex flex-col gap-[20px]">
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, phoneNumber: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                                placeholder="School Name"
                                value={newPlayerData.universityName}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, schoolName: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                            ...
                            <Form.Group controlId="formBirthplace" className=" mt-[10px] flex flex-col gap-[20px]">
                                ...
                                <Form.Control
                                    ...
                                    onChange={(e) => setNewPlayerData({ ...newPlayerData, birthplaceCity: e.target.value })}
                                />
                                <Form.Control
                                    type="text"
                                    placeholder="State"
                                    value={newPlayerData.birthplaceState}
                                    onChange={(e) => setNewPlayerData({ ...newPlayerData, birthplaceState: e.target.value })}
                                />
                            </Form.Group>
                        </Form.Group>

                        {/* Contact Details */}
                        <Form.Group controlId="formContactDetails" className=" mt-[10px] flex flex-col gap-[20px]">
                            <Form.Label>Contact Details</Form.Label>
                            <Form.Control
                                type="text"
                                placeholder="Twitter Link"
                                value={newPlayerData.twitterLink}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, twitterLink: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                                placeholder="Instagram Link"
                                value={newPlayerData.instagramLink}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, instagramLink: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                                placeholder="Facebook Link"
                                value={newPlayerData.facebookLink}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, facebookLink: e.target.value })}
                            />
                        </Form.Group>

                        {/* Academics */}
                        <Form.Group controlId="formAcademics" className=" mt-[10px] flex flex-col gap-[20px]">
                            <Form.Label>Academics</Form.Label>
                            <Form.Control
                                type="text"
                                placeholder="GPA"
                                value={newPlayerData.GPA}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, GPA: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                                placeholder="SAT Score"
                                value={newPlayerData.SATScore}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, SATScore: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                                placeholder="ACT Score"
                                value={newPlayerData.ACTScore}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, ACTScore: e.target.value })}
                            />
                            <Form.Control
                                type="text"
                                placeholder="NCAA ID"
                                value={newPlayerData.NCAAID}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, NCAAID: e.target.value })}
                            />
                        </Form.Group>

                        {/* Athletic Accomplishments */}
                        <Form.Group controlId="formAthleticAccomplishments" className=" mt-[10px] flex flex-col gap-[20px]">
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, athleticAccomplishments: e.target.value })}
                            />
                        </Form.Group>

                        {/* About Me */}
                        <Form.Group controlId="formAboutMe" className=" mt-[10px] flex flex-col gap-[20px]">
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, aboutMe: e.target.value })}
                            />
                        </Form.Group>

                        {/* Coach Information */}
                        <Form.Group controlId="formCoachInformation" className=" mt-[10px] flex flex-col gap-[20px]">
                            <Form.Label>Coach Information</Form.Label>
                            <Form.Control
                                type="text"
                                placeholder="Previous Coach Name"
                                value={newPlayerData.previousCoachName}
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, previousCoachName: e.target.value })}
                            />
                            <Form.Control
                                type="text"
...
/>
                        ...
                        <Form.Group controlId="formContactInformation" className=" mt-[10px] flex flex-col gap-[20px]">
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, auth:{
                                    ...newPlayerData.auth,
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, institute:{
                        ...
                        <Form.Group controlId="formContactDetails" className=" mt-[10px] flex flex-col gap-[20px]">
                            ...
                            <Form.Control
                                type="text"
                                placeholder="Twitter Link"
                                value={newPlayerData?.profile?.socialLinks?.find(u => u?.social_type == "twitter")?.link}
                                onChange={(e) => {
                                    const newLinkValue = e.target.value;
                                    setNewPlayerData((prevData) => {
                                        const socialLinks = prevData.profile.socialLinks || [];
                                        const twitterIndex = socialLinks.findIndex(link => link.social_type === 'twitter');
                                        
                                        if (twitterIndex !== -1) {
                                            // Update existing twitter link
                                            const updatedLinks = socialLinks.map((link, index) => 
                                                index === twitterIndex ? { ...link, link: newLinkValue } : link
                                            );
                                            return { ...prevData, profile: { ...prevData.profile, socialLinks: updatedLinks } };
                                        } else {
                                            // Add new twitter link
                                            return { ...prevData, profile: { ...prevData.profile, socialLinks: [...socialLinks, { social_type: 'twitter', link: newLinkValue }] } };
                                        }
                                    });
                                }}
                            />
                            <Form.Control
                                type="text"
                                placeholder="Instagram Link"
                                value={newPlayerData?.profile?.socialLinks?.find(u => u?.social_type == "instagram")?.link}
                                onChange={(e) => {
                                    const newLinkValue = e.target.value;
                                    setNewPlayerData((prevData) => {
                                        const socialLinks = prevData.profile.socialLinks || [];
                                        const instagramIndex = socialLinks?.findIndex(link => link?.social_type === 'instagram');
                                        
                                        if (instagramIndex !== -1) {
                                            // Update existing instagram link
                                            const updatedLinks = socialLinks?.map((link, index) => 
                                                index === instagramIndex ? { ...link, link: newLinkValue } : link
                                            );
                                            return { ...prevData, profile: { ...prevData.profile, socialLinks: updatedLinks } };
                                        } else {
                                            // Add new instagram link
                                            return { ...prevData, profile: { ...prevData.profile, socialLinks: [...socialLinks, { social_type: 'instagram', link: newLinkValue }] } };
                                        }
                                    });
                                }}
                            ...
                            <Form.Control
                                type="text"
                                placeholder="Facebook Link"
                                value={newPlayerData?.profile?.socialLinks?.find(u => u?.social_type == "facebook")?.link}
                                onChange={(e) => {
                                    const newLinkValue = e.target.value;
                                    setNewPlayerData((prevData) => {
                                        const socialLinks = prevData?.profile?.socialLinks || [];
                                        const facebookIndex = socialLinks?.findIndex(link => link?.social_type === 'facebook');
                                        
                                        if (facebookIndex !== -1) {
                                            // Update existing facebook link
                                            const updatedLinks = socialLinks?.map((link, index) => 
                                                index === facebookIndex ? { ...link, link: newLinkValue } : link
                                            );
                                            return { ...prevData, profile: { ...prevData.profile, socialLinks: updatedLinks } };
                                        } else {
                                            // Add new facebook link
                                            return { ...prevData, profile: { ...prevData.profile, socialLinks: [...socialLinks, { social_type: 'facebook', link: newLinkValue }] } };
                                        }
                                    });
                                }}
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => {
                                    ...
                                    setNewPlayerData((prevData) => {
                                        ...
                                        return { ...prevData, profile: { ...prevData.profile, academics: updatedAcademics } };
                                    });
                                }}
                            />
                            <Form.Control
                                ...
                                onChange={(e) => {
                                    ...
                                    setNewPlayerData((prevData) => {
                                        ...
                                        const updatedAcademics = academics.map((academic, index) =>
                                            index === 0 ? { ...academic, satScore: newSATScoreValue } : academic
                                        );
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => {
                                    ...
                                    setNewPlayerData((prevData) => {
                                        const academics = prevData.profile.academics || [{}];
                                        const updatedAcademics = academics.map((academic, index) =>
                                            index === 0 ? { ...academic, ncaaId: newNCAAIDValue } : academic
                                        );
                                        return {
                                            ...prevData,
                                            profile: {
                        ...
                        <Form.Group controlId="formAthleticAccomplishments" className=" mt-[10px] flex flex-col gap-[20px]">
                            ...
                            <Form.Control
                                ...
                                onChange={(e) => setNewPlayerData({ ...newPlayerData, profile:{
